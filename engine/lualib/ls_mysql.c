...
*/
static const char *getcolumntype (enum enum_field_types type) {
...
static void create_colinfo (lua_State *L, cur_data *cur) {
	...
	for (i = 1; i <= cur->numcols; i++) {
		...
		lua_rawseti (L, -3, i);
		sprintf (typenames, "%.20s(%ld)", getcolumntype (fields[i-1].type), fields[i-1].length);
		lua_pushstring(L, typenames);
		lua_rawseti (L, -2, i);
	}
